...
var callsActionsEnqueue = cli.Command{
	Name:    "enqueue",
	Usage:   "Places the call into a queue, where it waits until it is removed or bridged to\nanother leg. Queue behavior is configured through the request body.",
	Suggest: true,
	Flags: []cli.Flag{
		&requestflag.Flag[string]{
			Name:      "call-control-id",
...
var callsActionsGather = cli.Command{
	...
	Flags: []cli.Flag{
		...
		&requestflag.Flag[string]{
			...
			Usage:    "The digit used to terminate input if fewer than `maximum_digits` digits have been gathered. Set to an empty string to disable the terminating digit entirely, so that a digit such as `#` can be collected as input per `valid_digits`.",
			Default:  "#",
...
var callsActionsLeaveQueue = cli.Command{
	Name:    "leave-queue",
	Usage:   "Removes the call from the queue it is currently waiting in. The call remains\nactive and can be directed with further call commands.",
	Suggest: true,
	Flags: []cli.Flag{
		&requestflag.Flag[string]{
			Name:      "call-control-id",
...
var callsActionsPay = requestflag.WithInnerFlags(cli.Command{
	...
	Flags: []cli.Flag{
		...
		&requestflag.Flag[[]string]{
			...
			Usage:    "Restricts accepted card numbers to the listed card types. When the caller enters a card number that does not match one of the listed types, Pay treats the input as invalid and re-prompts for the card number. Cannot be used together with `payment_token`.",
			BodyPath: "valid_card_types",
		},
		&requestflag.Flag[string]{
			Name:     "voice",
...
var callsActionsStartAIAssistant = requestflag.WithInnerFlags(cli.Command{
	...
	Flags: []cli.Flag{
		...
		&requestflag.Flag[bool]{
			Name:     "send-message-history-updates",
			Usage:    "When `true`, a `call.ai_gather.message_history_updated` webhook carrying the full message history is sent each time the conversation message history is updated. The assistant's own `telephony_settings.send_message_history_updates` overrides this value when it is set.",
			Default:  false,
			BodyPath: "send_message_history_updates",
		},
		&requestflag.Flag[map[string]any]{
...
var callsActionsStopAIAssistant = cli.Command{
	...
	Usage:   "Stops the AI assistant currently engaged on the call. The call remains active\nand can continue with other call control commands.",
	Suggest: true,
	Flags: []cli.Flag{
		&requestflag.Flag[string]{
			Name:      "call-control-id",
...
var callsActionsStopTranscription = cli.Command{
	...
	Usage:   "Stops real-time transcription on the call. Transcription webhooks cease once the\ncommand takes effect; the call itself is unaffected.",
	Suggest: true,
	Flags: []cli.Flag{
		&requestflag.Flag[string]{
			Name:      "call-control-id",
