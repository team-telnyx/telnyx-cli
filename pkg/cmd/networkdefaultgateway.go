...
var networksDefaultGatewayCreate = cli.Command{
	...
	Flags: []cli.Flag{
		...
		&requestflag.Flag[string]{
			...
			Usage:    "Wireguard peer ID.",
			BodyPath: "wireguard_peer_id",
		},
	},
	Action:          handleNetworksDefaultGatewayCreate,
...
func handleNetworksDefaultGatewayCreate(ctx context.Context, cmd *cli.Command) error {
	client := telnyx.NewClient(getDefaultRequestOptions(cmd)...)
	unusedArgs := cmd.Args().Slice()
	if !cmd.IsSet("network-identifier") && len(unusedArgs) > 0 {
		cmd.Set("network-identifier", unusedArgs[0])
	...
	_, err = client.Networks.DefaultGateway.New(
		ctx,
		cmd.Value("network-identifier").(string),
		params,
		options...,
	)
	if err != nil {
...
func handleNetworksDefaultGatewayRetrieve(ctx context.Context, cmd *cli.Command) error {
	...
	var res []byte
	options = append(options, option.WithResponseBodyInto(&res))
	_, err = client.Networks.DefaultGateway.Get(ctx, cmd.Value("id").(string), options...)
	if err != nil {
		return err
	}
...
func handleNetworksDefaultGatewayDelete(ctx context.Context, cmd *cli.Command) error {
	...
	var res []byte
	options = append(options, option.WithResponseBodyInto(&res))
	_, err = client.Networks.DefaultGateway.Delete(ctx, cmd.Value("id").(string), options...)
	if err != nil {
		return err
	}
