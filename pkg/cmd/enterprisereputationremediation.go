...
var enterprisesReputationRemediationList = cli.Command{
	...
	Flags: []cli.Flag{
		...
		&requestflag.Flag[string]{
			Name:      "filter-status",
			Usage:     "Filter by customer-facing status.",
...
var enterprisesReputationRemediationSubmit = cli.Command{
	Name:    "submit",
	...
	Action:          handleEnterprisesReputationRemediationSubmit,
	HideHelpCommand: true,
}
...
func handleEnterprisesReputationRemediationSubmit(ctx context.Context, cmd *cli.Command) error {
	client := telnyx.NewClient(getDefaultRequestOptions(cmd)...)
	unusedArgs := cmd.Args().Slice()
	if !cmd.IsSet("enterprise-id") && len(unusedArgs) > 0 {
		cmd.Set("enterprise-id", unusedArgs[0])
	...
	params := telnyx.EnterpriseReputationRemediationSubmitParams{}
	...
	return ShowJSON(obj, ShowJSONOpts{
		...
		Format:         format,
		RawOutput:      cmd.Root().Bool("raw-output"),
		Title:          "enterprises:reputation:remediation submit",
		Transform:      transform,
	})
}
