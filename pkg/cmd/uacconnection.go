...
func handleUacConnectionsList(ctx context.Context, cmd *cli.Command) error {
	...
	if format == "raw" {
		var res []byte
		options = append(options, option.WithResponseBodyInto(&res))
		_, err = client.UacConnections.List(ctx, params, options...)
		if err != nil {
			return err
		}
		obj := gjson.ParseBytes(res)
		return ShowJSON(obj, ShowJSONOpts{
			...
			Format:         format,
			RawOutput:      cmd.Root().Bool("raw-output"),
			Title:          "uac-connections list",
			Transform:      transform,
		})
	} else {
		iter := client.UacConnections.ListAutoPaging(ctx, params, options...)
		maxItems := int64(-1)
...
func handleUacConnectionsDelete(ctx context.Context, cmd *cli.Command) error {
	client := telnyx.NewClient(getDefaultRequestOptions(cmd)...)
	unusedArgs := cmd.Args().Slice()
	if !cmd.IsSet("id") && len(unusedArgs) > 0 {
		cmd.Set("id", unusedArgs[0])
	...
	var res []byte
	options = append(options, option.WithResponseBodyInto(&res))
	_, err = client.UacConnections.Delete(ctx, cmd.Value("id").(string), options...)
	if err != nil {
		return err
	}
