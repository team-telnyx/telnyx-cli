...
func handleNotificationEventConditionsList(ctx context.Context, cmd *cli.Command) error {
	...
	if format == "raw" {
		...
		return ShowJSON(os.Stdout, os.Stderr, "notification-event-conditions list", obj, format, explicitFormat, transform)
	} else {
		...
		return ShowJSONIterator(os.Stdout, os.Stderr, "notification-event-conditions list", iter, format, explicitFormat, transform, maxItems)
	}
}
