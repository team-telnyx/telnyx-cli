...
var aiAssistantsCreate = requestflag.WithInnerFlags(cli.Command{
	...
	"telephony-settings": {
		...
		&requestflag.InnerFlag[bool]{
			Name:       "telephony-settings.send-message-history-updates",
			Usage:      "Whether the assistant sends a `call.ai_gather.message_history_updated` webhook with the full message history every time the conversation history changes. Leave unset to inherit the `send_message_history_updates` value from the `ai_assistant_start` or `gather_using_ai` command that started the conversation. Setting it here is authoritative: `true` turns the webhooks on even when the start command did not request them, and `false` turns them off even when it did. Messages exchanged during a private warm transfer acceptance phase are never included.",
			InnerField: "send_message_history_updates",
		},
		&requestflag.InnerFlag[bool]{
...
var aiAssistantsChat = cli.Command{
	...
	Flags: []cli.Flag{
		...
		&requestflag.Flag[bool]{
			Name:     "stream",
			Usage:    "When true, the response is streamed as Server-Sent Events (`text/event-stream`): `delta` events carry content fragments as they are generated, a final `done` event carries the full content plus `whatsapp_template`, and a terminal `error` event reports failures that happen after streaming started. When false (default), the response is a single JSON object.",
			Default:  false,
			BodyPath: "stream",
		},
	},
