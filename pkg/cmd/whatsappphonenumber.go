...
func handleWhatsappPhoneNumbersList(ctx context.Context, cmd *cli.Command) error {
	...
	if format == "raw" {
		...
		return ShowJSON(os.Stdout, "whatsapp:phone-numbers list", obj, format, transform)
	} else {
		...
		return ShowJSONIterator(os.Stdout, "whatsapp:phone-numbers list", iter, format, transform, maxItems)
	}
}
