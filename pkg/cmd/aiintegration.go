...
func handleAIIntegrationsRetrieve(ctx context.Context, cmd *cli.Command) error {
	...
	_, err = client.AI.Integrations.Get(ctx, cmd.Value("integration-id").(string), options...)
	if err != nil {
		return err
	}
	...
	return ShowJSON(obj, ShowJSONOpts{
		...
		Title:          "ai:integrations retrieve",
		Transform:      transform,
	})
}
...
func handleAIIntegrationsList(ctx context.Context, cmd *cli.Command) error {
	...
	var res []byte
	options = append(options, option.WithResponseBodyInto(&res))
	_, err = client.AI.Integrations.List(ctx, options...)
	if err != nil {
		return err
	}
