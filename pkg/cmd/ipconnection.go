...
var ipConnectionsCreate = requestflag.WithInnerFlags(cli.Command{
	...
	Action:          handleIPConnectionsCreate,
...
var ipConnectionsRetrieve = cli.Command{
	Name:    "retrieve",
	Usage:   "Retrieves the details of an existing ip connection.",
	Suggest: true,
	Flags: []cli.Flag{
		&requestflag.Flag[string]{
			Name:     "id",
	...
	Action:          handleIPConnectionsRetrieve,
	HideHelpCommand: true,
}

var ipConnectionsUpdate = requestflag.WithInnerFlags(cli.Command{
	Name:    "update",
	Usage:   "Updates settings of an existing IP connection.",
	Suggest: true,
	Flags: []cli.Flag{
		&requestflag.Flag[string]{
			Name:     "id",
	...
	Action:          handleIPConnectionsUpdate,
	HideHelpCommand: true,
}, map[string][]requestflag.HasOuterFlag{
	"inbound": {
		&requestflag.InnerFlag[string]{
		...
		&requestflag.InnerFlag[string]{
			Name:       "inbound.default-primary-ip-id",
			Usage:      "The default primary IP to use for the number. Only settable if the connection is\n              of IP authentication type. Value must be the ID of an authorized IP set on the connection.",
			InnerField: "default_primary_ip_id",
		},
		&requestflag.InnerFlag[string]{
			Name:       "inbound.default-routing-method",
			Usage:      "Default routing method to be used when a number is associated with the connection. Must be one of the routing method types or left blank, other values are not allowed.",
		...
		&requestflag.InnerFlag[string]{
			Name:       "inbound.default-secondary-ip-id",
			Usage:      "The default secondary IP to use for the number. Only settable if the connection is\n              of IP authentication type. Value must be the ID of an authorized IP set on the connection.",
			InnerField: "default_secondary_ip_id",
		},
		&requestflag.InnerFlag[string]{
			Name:       "inbound.default-tertiary-ip-id",
			Usage:      "The default tertiary IP to use for the number. Only settable if the connection is\n              of IP authentication type. Value must be the ID of an authorized IP set on the connection.",
			InnerField: "default_tertiary_ip_id",
		},
		&requestflag.InnerFlag[string]{
			Name:       "inbound.dnis-number-format",
			Usage:      `Allowed values: "+e164", "e164", "national", "sip_username".`,
...
var ipConnectionsDelete = cli.Command{
	...
	Action:          handleIPConnectionsDelete,
	HideHelpCommand: true,
}

func handleIPConnectionsCreate(ctx context.Context, cmd *cli.Command) error {
	...
	params := telnyx.IPConnectionNewParams{}
	...
	_, err = client.IPConnections.New(ctx, params, options...)
	if err != nil {
		return err
	}
...
func handleIPConnectionsRetrieve(ctx context.Context, cmd *cli.Command) error {
	...
	var res []byte
	options = append(options, option.WithResponseBodyInto(&res))
	_, err = client.IPConnections.Get(ctx, cmd.Value("id").(string), options...)
	if err != nil {
		return err
	}
...
func handleIPConnectionsUpdate(ctx context.Context, cmd *cli.Command) error {
	...
	params := telnyx.IPConnectionUpdateParams{}
...
func handleIPConnectionsList(ctx context.Context, cmd *cli.Command) error {
	...
	params := telnyx.IPConnectionListParams{}
	...
	if format == "raw" {
		var res []byte
		options = append(options, option.WithResponseBodyInto(&res))
		_, err = client.IPConnections.List(ctx, params, options...)
		if err != nil {
			return err
		}
		obj := gjson.ParseBytes(res)
...
func handleIPConnectionsDelete(ctx context.Context, cmd *cli.Command) error {
	client := telnyx.NewClient(getDefaultRequestOptions(cmd)...)
	unusedArgs := cmd.Args().Slice()
	if !cmd.IsSet("id") && len(unusedArgs) > 0 {
		cmd.Set("id", unusedArgs[0])
	...
	var res []byte
	options = append(options, option.WithResponseBodyInto(&res))
	_, err = client.IPConnections.Delete(ctx, cmd.Value("id").(string), options...)
	if err != nil {
		return err
	}
