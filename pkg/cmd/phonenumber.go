...
func handlePhoneNumbersDelete(ctx context.Context, cmd *cli.Command) error {
	...
	explicitFormat := cmd.Root().IsSet("format")
	transform := cmd.Root().String("transform")
	return ShowJSON(os.Stdout, os.Stderr, "phone-numbers delete", obj, format, explicitFormat, transform)
}
...
func handlePhoneNumbersSlimList(ctx context.Context, cmd *cli.Command) error {
	...
	if format == "raw" {
		...
		obj := gjson.ParseBytes(res)
		return ShowJSON(os.Stdout, os.Stderr, "phone-numbers slim-list", obj, format, explicitFormat, transform)
	} else {
		...
		return ShowJSONIterator(os.Stdout, os.Stderr, "phone-numbers slim-list", iter, format, explicitFormat, transform, maxItems)
	}
}
