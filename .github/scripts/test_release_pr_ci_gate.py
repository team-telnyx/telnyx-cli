#!/usr/bin/env python3
"""Static safety contracts for the DOT-2061 CLI rollout."""
...
class ReleasePRGateWorkflowTests(unittest.TestCase):
    def test_classifier_owns_every_full_ci_job(self):
        ...
        self.assertIn("python3 .github/scripts/test_verify_cli_release.py -v", ci)

    def test_publish_verifies_exact_release_assets_after_goreleaser(self):
        workflow = (ROOT / ".github/workflows/publish-release.yml").read_text()
        self.assertIn("name: verify release availability", workflow)
        self.assertIn("needs: goreleaser", workflow)
        self.assertIn("verify_cli_release.py", workflow)
